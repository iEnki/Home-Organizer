// Schritt-Definitionen für die interaktive Tour des Home Organizers
// Jeder Schritt: { target: "data-tour-id", title, text, position }

export const TOUR_STEPS = {
  dashboard: [
    {
      target: "tour-sidebar",
      titleKey: "tour:steps.dashboard.0.title",
      textKey: "tour:steps.dashboard.0.text",
      title: "Navigation",
      text: "Hier findest du alle Hauptbereiche des Home Organizers: Inventar, Vorräte, Geräte & Wartung, Einkauf, Aufgaben, Projekte, Budget, Suche, Wissen und Verlauf. Klicke auf ein Symbol, um den Bereich zu öffnen.",
      position: "right",
    },
    {
      target: "tour-dashboard-willkommen",
      titleKey: "tour:steps.dashboard.1.title",
      textKey: "tour:steps.dashboard.1.text",
      title: "Willkommen",
      text: "Diese Karte zeigt dir auf einen Blick, wie viele Aufgaben, Projekte oder Aktionen gerade anstehen – dein täglicher Überblick.",
      position: "bottom",
    },
    {
      target: "tour-dashboard-status",
      titleKey: "tour:steps.dashboard.2.title",
      textKey: "tour:steps.dashboard.2.text",
      title: "Haushalt-Status",
      text: "Hier siehst du den Gesamtstatus deines Haushalts: Vorräte, offene Aufgaben und aktive Projekte in einer kompakten Übersicht.",
      position: "bottom",
    },
    {
      target: "tour-dashboard-inventar",
      titleKey: "tour:steps.dashboard.3.title",
      textKey: "tour:steps.dashboard.3.text",
      title: "Schnellzugriff Inventar",
      text: "Schnellzugriff auf dein Inventar. Hier kannst du Gegenstände hinzufügen, bearbeiten oder filtern.",
      position: "bottom",
    },
    {
      target: "tour-dashboard-budget",
      titleKey: "tour:steps.dashboard.4.title",
      textKey: "tour:steps.dashboard.4.text",
      title: "Budget-Übersicht",
      text: "Öffnet die Budgetübersicht. Hier legst du monatliche Limits fest, erfasst Ausgaben und siehst deinen Fortschritt.",
      position: "bottom",
    },
    {
      target: "tour-dashboard-aufgaben",
      titleKey: "tour:steps.dashboard.5.title",
      textKey: "tour:steps.dashboard.5.text",
      title: "To-Dos",
      text: "Zeigt offene Aufgaben. Klicke hier, um deine Aufgaben zu verwalten oder neue Aufgaben hinzuzufügen.",
      position: "bottom",
    },
    {
      target: "tour-dashboard-bewohner",
      titleKey: "tour:steps.dashboard.6.title",
      textKey: "tour:steps.dashboard.6.text",
      title: "Bewohner",
      text: "Hier kannst du Bewohner verwalten und Aufgaben oder Inventar mit ihnen teilen.",
      position: "bottom",
    },
    {
      target: "tour-topbar-profil",
      titleKey: "tour:steps.dashboard.7.title",
      textKey: "tour:steps.dashboard.7.text",
      title: "Benutzerprofil",
      text: "Über das Benutzersymbol gelangst du zu deinem Profil, wo du dein Konto verwaltest, den Dark-Mode umschaltest und Anleitungen zurücksetzen kannst.",
      position: "bottom",
    },
  ],

  inventar: [
    {
      target: "tour-inventar-hinzufuegen",
      titleKey: "tour:steps.inventar.0.title",
      textKey: "tour:steps.inventar.0.text",
      title: "Neues Objekt hinzufügen",
      text: "Füge einen neuen Gegenstand hinzu. Gib Name, Kategorie, Standort und Beschreibung ein und speichere ihn in deinem Inventar.",
      position: "bottom",
    },
    {
      target: "tour-inventar-filter",
      titleKey: "tour:steps.inventar.1.title",
      textKey: "tour:steps.inventar.1.text",
      title: "Filter & Suche",
      text: "Nutze diese Filter, um Gegenstände nach Kategorie, Standort oder Status zu sortieren. Das Suchfeld hilft dir, ein bestimmtes Objekt schnell zu finden.",
      position: "bottom",
    },
    {
      target: "tour-inventar-liste",
      titleKey: "tour:steps.inventar.2.title",
      textKey: "tour:steps.inventar.2.text",
      title: "Dein Inventar",
      text: "Jede Karte repräsentiert einen Gegenstand. Über das Stift-Symbol bearbeitest du den Eintrag, über den Papierkorb löschst du ihn.",
      position: "top",
    },
  ],

  vorraete: [
    {
      target: "tour-vorraete-hinzufuegen",
      titleKey: "tour:steps.vorraete.0.title",
      textKey: "tour:steps.vorraete.0.text",
      title: "Neuen Vorrat hinzufügen",
      text: "Lege einen neuen Vorratsartikel an. Gib Name, aktuelle Menge, Mindestbestand und Standort an.",
      position: "bottom",
    },
    {
      target: "tour-vorraete-filter",
      titleKey: "tour:steps.vorraete.1.title",
      textKey: "tour:steps.vorraete.1.text",
      title: "Kategorie & Standort-Filter",
      text: "Sortiere deine Vorräte nach Kategorie oder Standort, um schnell zu sehen, was wo gelagert wird.",
      position: "bottom",
    },
    {
      target: "tour-vorraete-liste",
      titleKey: "tour:steps.vorraete.2.title",
      textKey: "tour:steps.vorraete.2.text",
      title: "Vorratsliste",
      text: "Hier siehst du alle Vorräte. Ein farbiger Indikator zeigt den Status: Grün = ausreichend, Gelb = knapp, Rot = leer oder unter Mindestbestand.",
      position: "top",
    },
    {
      target: "tour-vorraete-einkauf",
      titleKey: "tour:steps.vorraete.3.title",
      textKey: "tour:steps.vorraete.3.text",
      title: "Zur Einkaufsliste hinzufügen",
      text: "Artikel unter dem Mindestbestand können direkt zur Einkaufsliste hinzugefügt werden – so vergisst du nichts beim Einkaufen.",
      position: "left",
    },
  ],

  geraete: [
    {
      target: "tour-geraete-hinzufuegen",
      titleKey: "tour:steps.geraete.0.title",
      textKey: "tour:steps.geraete.0.text",
      title: "Neues Gerät hinzufügen",
      text: "Trage ein neues Gerät ein: Name, Kaufdatum, Standort, Garantiezeit und nächste Wartung – alles an einem Ort.",
      position: "bottom",
    },
    {
      target: "tour-geraete-liste",
      titleKey: "tour:steps.geraete.1.title",
      textKey: "tour:steps.geraete.1.text",
      title: "Geräteliste",
      text: "Liste aller registrierten Geräte mit Name, Standort, letztem Wartungsdatum und aktuellem Status.",
      position: "top",
    },
    {
      target: "tour-geraete-status",
      titleKey: "tour:steps.geraete.2.title",
      textKey: "tour:steps.geraete.2.text",
      title: "Wartungs-Status",
      text: "Die Farbmarkierung zeigt, ob ein Gerät gewartet werden muss: Grün = OK, Gelb = bald fällig, Rot = überfällig.",
      position: "left",
    },
  ],

  bewohner: [
    {
      target: "tour-bewohner-hinzufuegen",
      titleKey: "tour:steps.bewohner.0.title",
      textKey: "tour:steps.bewohner.0.text",
      title: "Bewohner hinzufügen",
      text: "Füge eine weitere Person zum Haushalt hinzu. Du kannst Aufgaben zuteilen und ihren Fortschritt verfolgen.",
      position: "bottom",
    },
    {
      target: "tour-bewohner-liste",
      titleKey: "tour:steps.bewohner.1.title",
      textKey: "tour:steps.bewohner.1.text",
      title: "Bewohnerliste",
      text: "Jede Karte zeigt Name und Aufgabenstatus eines Bewohners. Klicke auf eine Person, um ihr Aufgaben zuzuweisen.",
      position: "top",
    },
  ],

  einkaufliste: [
    {
      target: "tour-einkauf-hinzufuegen",
      titleKey: "tour:steps.einkaufliste.0.title",
      textKey: "tour:steps.einkaufliste.0.text",
      title: "Neuer Eintrag",
      text: "Füge einen neuen Artikel zur Einkaufsliste hinzu. Gib Name, Menge und optional eine Kategorie ein.",
      position: "bottom",
    },
    {
      target: "tour-einkauf-filter",
      titleKey: "tour:steps.einkaufliste.1.title",
      textKey: "tour:steps.einkaufliste.1.text",
      title: "Filter",
      text: "Blende erledigte Artikel aus, um deine Einkaufsliste übersichtlich zu halten.",
      position: "bottom",
    },
    {
      target: "tour-einkauf-liste",
      titleKey: "tour:steps.einkaufliste.2.title",
      textKey: "tour:steps.einkaufliste.2.text",
      title: "Einkaufsposten",
      text: "Zeigt alle offenen und abgehakten Artikel. Klicke auf das Häkchen, um einen Posten als erledigt zu markieren.",
      position: "top",
    },
  ],

  aufgaben: [
    {
      target: "tour-aufgaben-hinzufuegen",
      titleKey: "tour:steps.aufgaben.0.title",
      textKey: "tour:steps.aufgaben.0.text",
      title: "Neue Aufgabe",
      text: "Erstelle eine Aufgabe mit Titel, Kategorie, Priorität und Fälligkeit. Du kannst auch eine Notiz hinzufügen.",
      position: "bottom",
    },
    {
      target: "tour-aufgaben-filter",
      titleKey: "tour:steps.aufgaben.1.title",
      textKey: "tour:steps.aufgaben.1.text",
      title: "Filter",
      text: "Suche gezielt nach Aufgaben in bestimmten Bereichen oder mit bestimmtem Status (offen, in Bearbeitung, erledigt).",
      position: "bottom",
    },
    {
      target: "tour-aufgaben-liste",
      titleKey: "tour:steps.aufgaben.2.title",
      textKey: "tour:steps.aufgaben.2.text",
      title: "Aufgabenliste",
      text: "Zeigt alle Aufgaben. Klicke auf die Checkbox, um eine Aufgabe als erledigt zu markieren. Farben oder Icons kennzeichnen die Priorität.",
      position: "top",
    },
  ],

  projekte: [
    {
      target: "tour-projekte-hinzufuegen",
      titleKey: "tour:steps.projekte.0.title",
      textKey: "tour:steps.projekte.0.text",
      title: "Neues Projekt",
      text: "Lege ein Haushaltsprojekt an – z. B. Renovierung, Möbelkauf oder Gartenpflege. Definiere Titel, Budget, Start- und Enddatum.",
      position: "bottom",
    },
    {
      target: "tour-projekte-liste",
      titleKey: "tour:steps.projekte.1.title",
      textKey: "tour:steps.projekte.1.text",
      title: "Projektkarten",
      text: "Jede Karte zeigt Budgetfortschritt, Aufgabenstatus und wichtige Termine. Klicke auf eine Karte, um Details zu sehen.",
      position: "top",
    },
    {
      target: "tour-projekte-status",
      titleKey: "tour:steps.projekte.2.title",
      textKey: "tour:steps.projekte.2.text",
      title: "Statusindikatoren",
      text: "Farben oder Symbole zeigen, ob das Projekt im Plan ist, Verzögerungen hat oder das Budget überschreitet.",
      position: "left",
    },
  ],

  budget: [
    {
      target: "tour-budget-hinzufuegen",
      titleKey: "tour:steps.budget.0.title",
      textKey: "tour:steps.budget.0.text",
      title: "Ausgabe erfassen",
      text: "Erfasse eine neue Ausgabe. Wähle Kategorie, Datum und Betrag, um deine Haushaltskasse aktuell zu halten.",
      position: "bottom",
    },
    {
      target: "tour-budget-limits",
      titleKey: "tour:steps.budget.1.title",
      textKey: "tour:steps.budget.1.text",
      title: "Monatliche Limits",
      text: "Lege Budgetgrenzen pro Kategorie fest. Der Fortschrittsbalken zeigt, wie viel vom Budget bereits ausgegeben wurde.",
      position: "bottom",
    },
    {
      target: "tour-budget-uebersicht",
      titleKey: "tour:steps.budget.2.title",
      textKey: "tour:steps.budget.2.text",
      title: "Monatsübersicht",
      text: "Oben siehst du die aktuelle Übersicht mit Summen für Haushalt und Privat, deiner Buchungsanzahl und der gefilterten Liste darunter.",
      position: "bottom",
    },
    {
      target: "tour-budget-tabs",
      titleKey: "tour:steps.budget.3.title",
      textKey: "tour:steps.budget.3.text",
      title: "Reiter: Übersicht / Statistiken / Sparziele",
      text: "Wechsle zwischen Monatsübersicht, Statistiken mit Diagrammen (Jahres- und Monatsansicht) und deinen Sparzielen.",
      position: "bottom",
    },
    {
      target: "tour-budget-sparziele",
      titleKey: "tour:steps.budget.4.title",
      textKey: "tour:steps.budget.4.text",
      title: "Sparziele",
      text: "Lege Sparziele an (z. B. Urlaub, neue Waschmaschine). Du kannst Beträge zuweisen und den Fortschritt verfolgen.",
      position: "top",
    },
  ],

  suche: [
    {
      target: "tour-suche-feld",
      titleKey: "tour:steps.suche.0.title",
      textKey: "tour:steps.suche.0.text",
      title: "Globale Suche",
      text: "Gib einen Suchbegriff ein, um alle Bereiche gleichzeitig zu durchsuchen: Gegenstände, Vorräte, Aufgaben, Projekte und mehr.",
      position: "bottom",
    },
    {
      target: "tour-suche-tabs",
      titleKey: "tour:steps.suche.1.title",
      textKey: "tour:steps.suche.1.text",
      title: "Ergebnis-Kategorien",
      text: "Ergebnisse werden nach Kategorie gruppiert. Klicke auf einen Reiter, um nur Einträge aus diesem Bereich zu sehen.",
      position: "bottom",
    },
  ],
};
