...
import { Box, ChevronDown, ChevronUp, Info } from "lucide-react";
import { useTheme } from "../contexts/ThemeContext";

const itemCategories = [
  { id: "kartonStandard", volume: 0.06 },
  { id: "kartonBuch", volume: 0.04 },
  { id: "moebelGross", volume: 1.3 },
  { id: "moebelMittel", volume: 0.7 },
  { id: "moebelKlein", volume: 0.15 },
  { id: "geraetGross", volume: 0.5 },
  { id: "fahrrad", volume: 0.5 },
  { id: "pflanzeGross", volume: 0.2 },
];
...
const BedarfsrechnerVolumen = ({ onVolumeCalculated }) => {
  ...
  useEffect(() => {
    ...
    itemCategories.forEach((item) => {
      ...
      if (quantity > 0) currentTotalVolume += quantity * item.volume;
    });
    setTotalVolume(currentTotalVolume);
    if (onVolumeCalculated) onVolumeCalculated(currentTotalVolume);
  }, [quantities, onVolumeCalculated]);
    ...
    <div className="p-4 md:p-6 bg-light-card-bg dark:bg-dark-card-bg rounded-lg shadow border border-light-border dark:border-dark-border mt-6">
      <h2 className="text-xl font-semibold text-light-text-main dark:text-dark-text-main mb-1 flex items-center justify-between">
        <div className="flex items-center">
          ...
          {t("move:calculator.volume.title")}
        </div>
        <button
      ...
      {showHelp && (
        <div className="mb-4 p-3 bg-gray-50 dark:bg-dark-bg border border-gray-200 dark:border-dark-border rounded-md text-xs text-gray-600 dark:text-dark-text-secondary space-y-1">
          ...
          <ul className="list-disc list-inside pl-2">
            <li>{t("move:calculator.volume.help.smallVan")}</li>
            <li>{t("move:calculator.volume.help.mediumVan")}</li>
            <li>{t("move:calculator.volume.help.truck")}</li>
          </ul>
        </div>
      )}
        ...
        {itemCategories.map((item) => (
          <div key={item.id} className="grid grid-cols-3 gap-3 items-center">
            <label htmlFor={item.id} className={`${labelClass} col-span-2`}>
              {t(`move:calculator.volume.items.${item.id}`)}
            </label>
            <input
              type="number"
              id={item.id}
